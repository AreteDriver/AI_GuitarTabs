...
from tabsynth.fretboard import (
    build_fretboard, STANDARD_TUNING_HZ, cents_diff, hz_to_pitch_class
)
...
def match_chord_template(
    ...
    if not detected_classes.issubset(template.pitch_classes) and \
       not template.pitch_classes.issubset(detected_classes):
